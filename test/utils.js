...
function checkClaimedTokensAmount (
    ...
    currentTime,
    currentBalance,
    totalSupply, expectedValue
) {
    return instance.calculateEmissionTokens(
...
async function testTransferFrom (contract, granter, grantee, to, tokens) {
    ...
    const trasnferFromResponse = await contract.transferFrom.call(granter, to, tokens + 1, { from: grantee });
    ...
    const transferFromResponse = await contract.transferFrom.call(
        granter,
        to,
        tokens / 2,
        { from: grantee }
...
async function checkState (contracts, states) {
    for (const contractId in states) {
        ...
        for (const stateId in states[contractId]) {
